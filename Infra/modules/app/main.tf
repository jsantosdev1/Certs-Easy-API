...
resource "aws_lambda_function" "api_lambda" {
  function_name = "CertsEasy-${var.env}"
  ...
  filename      = "../../../target/CertsEasy-${var.app_version}-aws.jar"
  source_code_hash = filebase64sha256("../../../target/CertsEasy-${var.app_version}-aws.jar")
  publish = true
  timeout = 30
  snap_start {
    apply_on = "None"
...
resource "aws_apigatewayv2_api" "gateway" {
  name          = "certseasy-gateway-${var.env}"
  protocol_type = "HTTP"
  cors_configuration {
    allow_origins = ["*"]
...
resource "aws_cloudwatch_log_group" "api_gw_logs" {
  name = "/aws/api-gateway/${aws_apigatewayv2_api.gateway.name}"
  retention_in_days = 3
}
...
resource "aws_apigatewayv2_stage" "gateway_stage" {
  ...
  default_route_settings {
    ...
    throttling_rate_limit  = 90
  }
}

resource "aws_apigatewayv2_integration" "lambda_integration_criar" {
  api_id           = aws_apigatewayv2_api.gateway.id
  ...
  request_parameters = {
    "overwrite:header.spring.cloud.function.definition" = "processarCertificado"
  }
}

resource "aws_apigatewayv2_route" "create_cert" {
  api_id    = aws_apigatewayv2_api.gateway.id
  route_key = "POST /v1/certificados"
  target    = "integrations/${aws_apigatewayv2_integration.lambda_integration_criar.id}"
}
